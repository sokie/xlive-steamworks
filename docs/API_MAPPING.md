...
| XSessionSearch, XSessionSearchEx | middle | `RequestLobbyList` with kind and title filters, plus one filter per XLAST query filter: the attribute's lobby key compared with the parameter, constant or context value the query names (numeric when the value fits an int, text otherwise). Without an XLAST query every context and property passed is an equality filter. A result carries the attributes the query declares, in the declared order, plus X_PROPERTY_GAMER_HOSTNAME after them when the query leaves it out. Without a query it carries every attribute the host published. The host's gamertag and XUID are published as X_PROPERTY_GAMER_HOSTNAME and X_PROPERTY_GAMER_PUID |
