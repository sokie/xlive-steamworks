...
// Titles set most contexts and properties after XSessionCreate. Lobby data is per key, so the
// host pushes only the changed key.
...
void OnLobbyDataUpdate(const LobbyDataUpdate_t& update)
{
	...
	if (update.m_ulSteamIDLobby == update.m_ulSteamIDMember) {
		...
		if (session && !session->host) {
			// The host may have migrated or changed the slots, so refresh the local copy.
			XSESSION_INFO info;
			if (ReadSessionInfo(lobby, &info)) {
				session->info = info;
			}
