...
// The link time from the PE header, since __DATE__ only changes when this file recompiles.
...
{
	XLS_TRACE_FN();
	// A Steam user cannot sign out from inside the game, so this only completes the overlapped.
	return xls::OverlappedReturn(pOverlapped, ERROR_SUCCESS) == ERROR_IO_PENDING ? HRESULT_FROM_WIN32(ERROR_IO_PENDING) : S_OK;
}
